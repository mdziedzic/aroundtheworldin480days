...
//					'img' => ( !empty($thumb) ) ? '<a href="'.$permalink.'" title="'. esc_attr($title) .'" target="' . $this->user_settings['tools']['link']['target'] . '">' . $thumb . '</a>' : '',
					'img' => ( !empty($thumb) ) ? '<a href="'.$permalink.'" title="'. esc_attr($title) .'">' . $thumb . '</a>' : '',
		...
		protected function _get_stats($p, $instance) {
			...
			if ($instance['stats_tag']['views']) {
				...
				if ($instance['order_by'] == 'avg') {
					...
					$views_text = sprintf(
//					_n('1 view', '%s views', $pageviews, $this->plugin_slug),  // eggfoo
					_n('1', '%s', $pageviews, $this->plugin_slug),  // eggfoo
