GeoMashup.loadFullPost=function(point){var i,request,cache,objects,object_ids;objects=this.getObjectsAtLocation(point);object_ids=this.getOnObjectIDs(objects);cache=this.locationCache(point,"full-post-"+object_ids.join(","));if(cache.html)this.getShowPostElement().innerHTML=cache.html;else{this.getShowPostElement().innerHTML='<div align="center"><img src="'+this.opts.url_path+'/images/busy_icon.gif" alt="Loading..." /></div>';request={url:this.geo_query_url+"&object_name="+this.opts.object_name+"&object_ids="+
object_ids.join(",")+"&template=full-post"};this.doAction("fullPostRequest",objects,request);jQuery.get(request.url,function(content){var filter={content:content};GeoMashup.doAction("fullPostLoad",objects,filter);cache.html=filter.content;jQuery(GeoMashup.getShowPostElement()).html(filter.content);GeoMashup.doAction("fullPostChanged")})}};
...
options);if(this.map.getZoom()>term_data.max_line_zoom)this.hideLine(term_data.line)};
GeoMashup.openInfoWindow=function(marker){var request,cache,object_ids,i,object_element,point=marker.location;if(this.open_window_marker&&!this.opts.multiple_info_windows)this.open_window_marker.closeBubble();object_ids=this.getOnObjectIDs(this.getObjectsAtLocation(point));cache=this.locationCache(point,"info-window-"+object_ids.join(","));if(cache.html){marker.setInfoBubble(cache.html);marker.openBubble()}else{marker.setInfoBubble('<div align="center"><img src="'+this.opts.url_path+'/images/busy_icon.gif" alt="Loading..." /></div>');
marker.openBubble();this.open_window_marker=marker;request={url:this.geo_query_url+"&object_name="+this.opts.object_name+"&object_ids="+object_ids.join(",")};this.doAction("markerInfoWindowRequest",marker,request);jQuery.get(request.url,function(content){var filter={content:content};marker.closeBubble();GeoMashup.doAction("markerInfoWindowLoad",marker,filter);cache.html=GeoMashup.parentizeLinksMarkup(filter.content);marker.setInfoBubble(cache.html);marker.openBubble()})}};
GeoMashup.closeInfoWindow=function(marker){marker.closeBubble()};GeoMashup.addGlowMarker=function(marker){var point=marker.location,glow_options={clickable:true,icon:this.opts.url_path+"/images/mm_36_glow.png",iconSize:[30,36],iconAnchor:[15,36]};if(this.glow_marker)this.removeGlowMarker();this.doAction("glowMarkerIcon",this.opts,glow_options);this.glow_marker=new mxn.Marker(point);this.glow_marker.addData(glow_options);this.glow_marker.click.addHandler(function(){GeoMashup.deselectMarker()});this.map.addMarker(this.glow_marker)};
GeoMashup.removeGlowMarker=function(){if(this.glow_marker){this.glow_marker.hide();this.map.removeMarker(this.glow_marker);this.glow_marker=null}};GeoMashup.hideAttachments=function(){var i,j,obj};
GeoMashup.showMarkerAttachments=function(marker){var object_ids,uncached_ids=[];this.hideAttachments();object_ids=this.getOnObjectIDs(this.getMarkerObjects(marker));jQuery.each(object_ids,function(i,id){var cached_attachments=GeoMashup.locationCache(marker.location,"attachments-"+id);if(cached_attachments.urls)jQuery.each(cached_attachments.urls,function(j,url){GeoMashup.open_attachments.push(url);GeoMashup.map.addOverlay(url)});else uncached_ids.push(id)});jQuery.each(uncached_ids,function(i,id){var ajax_params=
{action:"geo_mashup_kml_attachments"};ajax_params.post_ids=id;jQuery.getJSON(GeoMashup.opts.ajaxurl+"?callback=?",ajax_params,function(data){var cached_attachments=GeoMashup.locationCache(marker.location,"attachments-"+id);if(!cached_attachments.urls)cached_attachments.urls=[];jQuery.each(data,function(j,url){cached_attachments.urls.push(url);GeoMashup.open_attachments.push(url);GeoMashup.map.addOverlay(url)})})})};
GeoMashup.addObjectIcon=function(obj){if(typeof customGeoMashupCategoryIcon==="function"&&obj.terms&&obj.terms.hasOwnProperty("category"))obj.icon=customGeoMashupCategoryIcon(GeoMashup.opts,obj.terms.category);if(!obj.icon){jQuery.each(obj.terms,function(taxonomy,terms){var single_icon;if(terms.length>1){obj.icon=GeoMashup.clone(GeoMashup.multiple_term_icon);return false}else if(terms.length===1){single_icon=GeoMashup.term_manager.getTermData(taxonomy,terms[0],"icon");if(obj.icon&&obj.icon.image!==
single_icon.image){obj.icon=GeoMashup.clone(GeoMashup.multiple_term_icon);return false}else obj.icon=GeoMashup.clone(single_icon)}});if(!obj.icon)obj.icon=GeoMashup.colorIcon("red");this.doAction("objectIcon",GeoMashup.opts,obj)}};
GeoMashup.createMarker=function(point,obj){var marker,marker_opts;if(!obj.icon)this.addObjectIcon(obj);marker_opts={label:obj.title,icon:obj.icon.image,iconSize:obj.icon.iconSize,iconShadow:obj.icon.iconShadow,iconAnchor:obj.icon.iconAnchor,iconShadowSize:obj.icon.shadowSize,visible:true};this.doAction("objectMarkerOptions",this.opts,marker_opts,obj);marker=new mxn.Marker(point);marker.addData(marker_opts);marker.click.addHandler(function(){if(marker===GeoMashup.selected_marker)GeoMashup.deselectMarker();
else GeoMashup.selectMarker(marker)});this.doAction("marker",this.opts,marker);return marker};GeoMashup.clickObjectMarker=function(object_id,try_count){var obj=this.objects[object_id];if(!GeoMashup.isObjectOn(obj))return false;if(typeof try_count==="undefined")try_count=1;if(obj&&obj.marker&&try_count<4)if(try_count<2){try_count+=1;setTimeout(function(){GeoMashup.clickObjectMarker(object_id,try_count)},1E3)}else obj.marker.click.fire()};
GeoMashup.colorIcon=function(color_name){var icon=this.clone(this.base_color_icon);icon.image=this.opts.url_path+"/images/mm_36_"+color_name+".png";return icon};GeoMashup.getMarkerLatLng=function(marker){return marker.location};GeoMashup.hideMarker=function(marker){if(marker===this.selected_marker)this.deselectMarker();marker.hide()};GeoMashup.showMarker=function(marker){marker.show()};
GeoMashup.hideLine=function(line){try{line.hide()}catch(e){this.map.removePolyline(line)}line.setAttribute("visible",false)};GeoMashup.showLine=function(line){try{line.show()}catch(e){this.map.addPolyline(line)}line.setAttribute("visible",true)};GeoMashup.isLineVisible=function(line){return line.getAttribute("visible")};GeoMashup.newLatLng=function(lat,lng){return new mxn.LatLonPoint(lat,lng)};
GeoMashup.extendLocationBounds=function(latlng){if(this.location_bounds)this.location_bounds.extend(latlng);else this.location_bounds=new mxn.BoundingBox(latlng,latlng)};GeoMashup.addMarkers=function(markers){this.forEach(markers,function(i,marker){this.map.addMarker(marker)})};
GeoMashup.makeMarkerMultiple=function(marker){var plus_image,original_image;if(typeof customGeoMashupMultiplePostImage==="function")plus_image=customGeoMashupMultiplePostImage(this.opts,marker);if(!plus_image)plus_image=this.opts.url_path+"/images/mm_36_plus.png";original_image=marker.iconUrl;marker.setIcon(plus_image);this.doAction("multiObjectMarker",this.opts,marker);this.doAction("multiObjectIcon",this.opts,plus_image);if(marker.onmap&&marker.iconUrl!==original_image){this.map.removeMarker(marker);
this.map.addMarker(marker)}};GeoMashup.setMarkerImage=function(marker,image_url){if(marker.iconUrl!==image_url){marker.setIcon(image_url);if(marker.onmap){this.map.removeMarker(marker);this.map.addMarker(marker)}}};
